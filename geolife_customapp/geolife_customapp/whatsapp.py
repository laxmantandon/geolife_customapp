...
def send_ledger_whatsapp():
    ...
    try:
        customers = frappe.db.sql("SELECT name, customer_name, mobile_no, customer_primary_contact from tabCustomer where mobile_no is not null and mobile_no <> '' and branch not in ('Amazon', 'Karnataka Factory', 'Factory')", as_dict=1)
        company = 'Geolife Agritech India Private Limited'
